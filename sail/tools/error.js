;(function () {
    ...
    _proto.checkError = function(cmd, data, code, errormsg, type){
        if(type == "ajax"){
            //ajax网络异常，包含超时和所有异常
            if(cmd == "xhr.error"){
                this.dispathError("xhrError",data);
                return true;
            }
            ...
            //输分提醒
            if(window.GM && GM.loseRemind && GM.loseRemind.check(data)){
                this.dispathError("loseRemind",cmd);
                return true;
            }
    ...
    _proto.dispathError = function(code,cmd,msg,data) {
        switch(code.toString()){
            ...
            case "xhrError" : 
                ...
                break;
            case "1000":
                if(cmd == GAME_CMDS.GET_RANK_BET || cmd == GAME_CMDS.GET_RANK_PRIZE || cmd == GAME_CMDS.GET_RANK_MINE){
                    Sail.io.publish(GAME_CMDS.RANK_NO_DATA, {}, cmd);
                }
                if(cmd == GAME_CMDS.GET_RANK_RICH){
                    Sail.io.publish(GAME_CMDS.RANK_RICH_NODATA, {}, cmd);
                }
                break;
            default : 
                if(cmd == GAME_CMDS.GET_GAME_RESULT){
